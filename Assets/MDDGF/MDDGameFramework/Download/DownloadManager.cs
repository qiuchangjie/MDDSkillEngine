...
namespace MDDGameFramework
{
    ...
    internal sealed partial class DownloadManager : MDDGameFrameworkModule, IDownloadManager
    {
        ...
        public int AddDownload(string downloadPath, string downloadUri, string tag, int priority, object userData)
        {
            ...
            {
                throw new MDDGameFrameworkException("Download path is invalid.");
            }

            if (string.IsNullOrEmpty(downloadUri))
            {
                throw new MDDGameFrameworkException("Download uri is invalid.");
            }

            if (TotalAgentCount <= 0)
            {
                throw new MDDGameFrameworkException("You must add download agent first.");
            }
