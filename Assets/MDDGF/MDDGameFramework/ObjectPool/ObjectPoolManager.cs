...
namespace MDDGameFramework
{
    ...
    internal sealed partial class ObjectPoolManager : MDDGameFrameworkModule, IObjectPoolManager
    {
        ...
        public bool HasObjectPool(Type objectType)
        {
            if (objectType == null)
            {
                throw new MDDGameFrameworkException("Object type is invalid.");
            }

            if (!typeof(ObjectBase).IsAssignableFrom(objectType))
            {
                throw new MDDGameFrameworkException(Utility.Text.Format("Object type '{0}' is invalid.", objectType.FullName));
            }
        ...
        public bool DestroyObjectPool<T>(IObjectPool<T> objectPool) where T : ObjectBase
        {
            if (objectPool == null)
            {
                throw new MDDGameFrameworkException("Object pool is invalid.");
            }
        ...
        private IObjectPool<T> InternalCreateObjectPool<T>(string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity, float expireTime, int priority) where T : ObjectBase
        {
            ...
            {
                throw new MDDGameFrameworkException(Utility.Text.Format("Already exist object pool '{0}'.", typeNamePair));
            }
