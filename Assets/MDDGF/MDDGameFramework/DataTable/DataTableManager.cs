...
namespace MDDGameFramework
{
    ...
    internal sealed partial class DataTableManager : MDDGameFrameworkModule, IDataTableManager
    {
        ...
        public void SetDataTableHelper(IDataTableHelper dataTableHelper)
        {
            ...
            {
                throw new MDDGameFrameworkException("Data table helper is invalid.");
            }
        ...
        public bool HasDataTable(Type dataRowType)
        {
            if (dataRowType == null)
            {
                throw new MDDGameFrameworkException("Data row type is invalid.");
            }

            if (!typeof(IDataRow).IsAssignableFrom(dataRowType))
            {
                throw new MDDGameFrameworkException(Utility.Text.Format("Data row type '{0}' is invalid.", dataRowType.FullName));
            }
        ...
        public IDataTable<T> CreateDataTable<T>(string name) where T : class, IDataRow, new()
        {
            ...
            {
                throw new MDDGameFrameworkException(Utility.Text.Format("Already exist data table '{0}'.", typeNamePair.ToString()));
            }
        ...
        public bool DestroyDataTable<T>(IDataTable<T> dataTable) where T : IDataRow
        {
            if (dataTable == null)
            {
                throw new MDDGameFrameworkException("Data table is invalid.");
            }
