...
{
    internal sealed partial class DataTableManager : MDDGameFrameworkModule, IDataTableManager
    {
        /// <summary>
        /// 数据表。
        /// </summary>
        ...
        private sealed class DataTable<T> : DataTableBase, IDataTable<T> where T : class, IDataRow, new()
        {
            ...
            public bool HasDataRow(Predicate<T> condition)
            {
                if (condition == null)
                {
                    throw new MDDGameFrameworkException("Condition is invalid.");
                }
            ...
            public T[] GetDataRows(Comparison<T> comparison)
            {
                if (comparison == null)
                {
                    throw new MDDGameFrameworkException("Comparison is invalid.");
                }
            ...
            public override bool AddDataRow(string dataRowString, object userData)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    throw new MDDGameFrameworkException(Utility.Text.Format("Can not parse data row string for data table '{0}' with exception '{1}'.", new TypeNamePair(typeof(T), Name).ToString(), exception.ToString()), exception);
                }
            }
            ...
            public override bool AddDataRow(byte[] dataRowBytes, int startIndex, int length, object userData)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    throw new MDDGameFrameworkException(Utility.Text.Format("Can not parse data row bytes for data table '{0}' with exception '{1}'.", new TypeNamePair(typeof(T), Name).ToString(), exception.ToString()), exception);
                }
            }
            ...
            private void InternalAddDataRow(T dataRow)
            {
                ...
                {
                    throw new MDDGameFrameworkException(Utility.Text.Format("Already exist '{0}' in data table '{1}'.", dataRow.Id.ToString(), new TypeNamePair(typeof(T), Name).ToString()));
                }
