...
namespace MDDGameFramework
{
    ...
    internal sealed class DataProvider<T> : IDataProvider<T>
    {
        ...
        public static void EnsureCachedBytesSize(int ensureSize)
        {
            if (ensureSize < 0)
            {
                throw new MDDGameFrameworkException("Ensure size is invalid.");
            }
        ...
        public void ReadData(string dataAssetName, int priority, object userData)
        {
            ...
            {
                throw new MDDGameFrameworkException("You must set data provider helper first.");
            }
            ...
            switch (result)
            {
                ...
                case HasAssetResult.BinaryOnFileSystem:
                    ...
                    {
                        throw new MDDGameFrameworkException(Utility.Text.Format("Load binary '{0}' from file system with internal error.", dataAssetName));
                    }
                    ...
                    try
                    {
                        ...
                        {
                            throw new MDDGameFrameworkException(Utility.Text.Format("Load data failure in data provider helper, data asset name '{0}'.", dataAssetName));
                        }
                ...
                default:
                    throw new MDDGameFrameworkException(Utility.Text.Format("Data asset '{0}' is '{1}'.", dataAssetName, result.ToString()));
            }
        }
        ...
        public bool ParseData(string dataString, object userData)
        {
            if (m_DataProviderHelper == null)
            {
                throw new MDDGameFrameworkException("You must set data helper first.");
            }

            if (dataString == null)
            {
                throw new MDDGameFrameworkException("Data string is invalid.");
            }
            ...
            catch (Exception exception)
            {
                ...
                throw new MDDGameFrameworkException(Utility.Text.Format("Can not parse data string with exception '{0}'.", exception.ToString()), exception);
            }
        }
        ...
        public bool ParseData(byte[] dataBytes)
        {
            if (dataBytes == null)
            {
                throw new MDDGameFrameworkException("Data bytes is invalid.");
            }
        ...
        public bool ParseData(byte[] dataBytes, int startIndex, int length, object userData)
        {
            ...
            catch (Exception exception)
            {
                ...
                throw new MDDGameFrameworkException(Utility.Text.Format("Can not parse data bytes with exception '{0}'.", exception.ToString()), exception);
            }
        }
        ...
        internal void SetDataProviderHelper(IDataProviderHelper<T> dataProviderHelper)
        {
            ...
            {
                throw new MDDGameFrameworkException("Data provider helper is invalid.");
            }
