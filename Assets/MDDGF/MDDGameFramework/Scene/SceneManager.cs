...
namespace MDDGameFramework
{
    ...
    /// 场景管理器。
    /// </summary>
    internal sealed class SceneManager : MDDGameFrameworkModule, ISceneManager
    {
        ...
        public void LoadScene(string sceneAssetName, int priority, object userData)
        {
            ...
            {
                throw new MDDGameFrameworkException(Utility.Text.Format("Scene asset '{0}' is being unloaded.", sceneAssetName));
            }

            if (SceneIsLoading(sceneAssetName))
            {
                throw new MDDGameFrameworkException(Utility.Text.Format("Scene asset '{0}' is being loaded.", sceneAssetName));
            }

            if (SceneIsLoaded(sceneAssetName))
            {
                throw new MDDGameFrameworkException(Utility.Text.Format("Scene asset '{0}' is already loaded.", sceneAssetName));
            }
        ...
        public void UnloadScene(string sceneAssetName, object userData)
        {
            ...
            if (!SceneIsLoaded(sceneAssetName))
            {
                throw new MDDGameFrameworkException(Utility.Text.Format("Scene asset '{0}' is not loaded yet.", sceneAssetName));
            }
        ...
        private void LoadSceneFailureCallback(string sceneAssetName, string status, string errorMessage, object userData)
        {
            m_LoadingSceneAssetNames.Remove(sceneAssetName);
        ...
        private void UnloadSceneFailureCallback(string sceneAssetName, object userData)
        {
            ...
            throw new MDDGameFrameworkException(Utility.Text.Format("Unload scene failure, scene asset name '{0}'.", sceneAssetName));
        }
    }
}
