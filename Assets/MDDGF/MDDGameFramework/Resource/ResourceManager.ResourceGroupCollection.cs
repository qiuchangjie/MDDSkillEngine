    ...
    internal sealed partial class ResourceManager : MDDGameFrameworkModule, IResourceManager
    {
        ...
        private sealed class ResourceGroupCollection : IResourceGroupCollection
        {
            ...
            public ResourceGroupCollection(ResourceGroup[] resourceGroups, Dictionary<ResourceName, ResourceInfo> resourceInfos)
            {
                ...
                {
                    throw new MDDGameFrameworkException("Resource groups is invalid.");
                }

                if (resourceInfos == null)
                {
                    throw new MDDGameFrameworkException("Resource infos is invalid.");
                }
                ...
                for (int i = 0; i < lastIndex; i++)
                {
                    ...
                    {
                        throw new MDDGameFrameworkException(Utility.Text.Format("Resource group index '{0}' is invalid.", i));
                    }
                    ...
                    for (int j = i + 1; j < resourceGroups.Length; j++)
                    {
                        ...
                        {
                            throw new MDDGameFrameworkException(Utility.Text.Format("Resource group '{0}' duplicated.", resourceGroups[i].Name));
                        }
                    }
                }
                ...
                {
                    throw new MDDGameFrameworkException(Utility.Text.Format("Resource group index '{0}' is invalid.", lastIndex));
                }
                ...
                foreach (ResourceGroup resourceGroup in m_ResourceGroups)
                {
                    ...
                    foreach (ResourceName resourceName in cachedResourceNames)
                    {
                        ...
                        {
                            throw new MDDGameFrameworkException(Utility.Text.Format("Resource info '{0}' is invalid.", resourceName.FullName));
                        }
