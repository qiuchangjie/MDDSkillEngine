...
namespace MDDGameFramework
{
    internal sealed partial class ResourceManager : MDDGameFrameworkModule, IResourceManager
    {
        ...
        private sealed class ResourceIniter
        {
            ...
            public MDDGameFrameworkAction ResourceInitComplete;
            ...
            private void OnLoadPackageVersionListSuccess(string fileUri, byte[] bytes, float duration, object userData)
            {
                ...
                try
                {
                    ...
                    {
                        throw new MDDGameFrameworkException("Deserialize package version list failure.");
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new MDDGameFrameworkException(Utility.Text.Format("Parse package version list exception '{0}'.", exception), exception);
                }
                finally
                {
            ...
            {
                throw new MDDGameFrameworkException(Utility.Text.Format("Package version list '{0}' is invalid, error message is '{1}'.", fileUri, string.IsNullOrEmpty(errorMessage) ? "<Empty>" : errorMessage));
            }
        }
    }
}
