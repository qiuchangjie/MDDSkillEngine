...
namespace MDDGameFramework.Editor.ResourceTools
{
    public sealed partial class ResourceBuilderController
    {
        ...
        {
            m_ConfigurationPath = Type.GetConfigurationPath<ResourceBuilderConfigPathAttribute>() ?? Utility.Path.GetRegularPath(Path.Combine(Application.dataPath, "MDDSkillEngine/Configs/ResourceBuilder.xml"));
        ...
        public string GameFrameworkVersion
        {
            get
            {
                return MDDGameFramework.Version.GameFrameworkVersion;
            }
        }
        ...
        public event MDDGameFrameworkAction<int, int> OnLoadingResource = null;

        public event MDDGameFrameworkAction<int, int> OnLoadingAsset = null;

        public event MDDGameFrameworkAction OnLoadCompleted = null;

        public event MDDGameFrameworkAction<int, int> OnAnalyzingAsset = null;

        public event MDDGameFrameworkAction OnAnalyzeCompleted = null;
        ...
        public event MDDGameFrameworkAction<Platform> ProcessResourceComplete = null;

        public event MDDGameFrameworkAction<string> BuildResourceError = null;
        ...
        public bool BuildResources()
        {
            ...
            finally
            {
                ...
                {
                    MDDGameFrameworkEntry.Shutdown();
                    m_FileSystemManager = null;
                }
            }
        }
        ...
        private void ProcessPackageVersionList(string outputPackagePath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(packageVersionListPath, FileMode.Create, FileAccess.Write))
            {
                ...
                {
                    throw new MDDGameFrameworkException("Serialize package version list failure.");
                }
            }
        }
        ...
        private VersionListData ProcessUpdatableVersionList(string outputFullPath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(updatableVersionListPath, FileMode.Create, FileAccess.Write))
            {
                if (!serializer.Serialize(fileStream, versionList))
                {
                    throw new MDDGameFrameworkException("Serialize updatable version list failure.");
                }
            }
        ...
        private void ProcessReadOnlyVersionList(string outputPackedPath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(readOnlyVersionListPath, FileMode.Create, FileAccess.Write))
            {
                if (!serializer.Serialize(fileStream, versionList))
                {
                    throw new MDDGameFrameworkException("Serialize read-only version list failure.");
                }
            }
        }
        ...
        private void CreateFileSystems(IEnumerable<ResourceData> resourceDatas, string outputPath, Dictionary<string, IFileSystem> outputFileSystem)
        {
            ...
            {
                m_FileSystemManager = MDDGameFrameworkEntry.GetModule<IFileSystemManager>();
        ...
        private static BuildTarget GetBuildTarget(Platform platform)
        {
            switch (platform)
            {
                ...
                default:
                    throw new MDDGameFrameworkException("Platform is invalid.");
            }
        }
