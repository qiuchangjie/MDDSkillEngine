...
namespace Slate
{
    ...
    {
        protected T action { get { return (T)target; } }
    }
    ...
    public class ActionClipInspector : OdinEditor
    {
        ...
        void ShowInOutControls() {
            ...
            if ( action.CanCrossBlend(previousClip) ) { previousTime -= Mathf.Min(action.length / 2, ( previousClip.endTime - previousClip.startTime ) / 2); }

            var nextClip = action.GetNextSibling();
            var nextTime = nextClip != null ? nextClip.startTime : action.parent.endTime;
            if ( action.CanCrossBlend(nextClip) ) { nextTime += Mathf.Min(action.length / 2, ( nextClip.endTime - nextClip.startTime ) / 2); }
