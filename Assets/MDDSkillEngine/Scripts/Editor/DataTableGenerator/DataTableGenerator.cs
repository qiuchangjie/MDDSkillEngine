...
namespace MDDSkillEngine
{
    public sealed class DataTableGenerator
    {
        ...
        {
            string binaryDataFileName = Utility.Path.GetRegularPath(Path.Combine(DataTablePathBytes, dataTableName + ".bytes"));
        ...
        private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
        {
            ...
            codeContent.Replace("__DATA_TABLE_NAME_SPACE__", "MDDSkillEngine");
        ...
        private static string GenerateDataTablePropertyArray(DataTableProcessor dataTableProcessor)
        {
            ...
            foreach (PropertyCollection propertyCollection in propertyCollections)
            {
                ...
                stringBuilder
                    ...
                    .AppendLine("            }")
                    .AppendLine()
                    .AppendFormat("            throw new MDDGameFrameworkException(Utility.Text.Format(\"Get{0} with invalid id '{{0}}'.\", id.ToString()));", propertyCollection.Name).AppendLine()
                    .AppendLine("        }")
                    .AppendLine()
                    ...
                    .AppendLine("            {")
                    .AppendFormat("                throw new MDDGameFrameworkException(Utility.Text.Format(\"Get{0}At with invalid index '{{0}}'.\", index.ToString()));", propertyCollection.Name).AppendLine()
                    .AppendLine("            }")
                    .AppendLine()
        ...
        private sealed class PropertyCollection
        {
            ...
            public KeyValuePair<int, string> GetItem(int index)
            {
                ...
                {
                    throw new MDDGameFrameworkException(Utility.Text.Format("GetItem with invalid index '{0}'.", index.ToString()));
                }
